...
static void daccb(DACDriver *dacp) {
  (void)dacp;
  palTogglePad(GPIOC, GPIOC_LED1); // Green
}
...
static void dacerrcb(DACDriver *dacp) {
  (void)dacp;
  palTogglePad(GPIOC, GPIOC_LED2); // Orange
}
...
static const DACConfig daccfg3 = {
...
int main(void) {
   ...
   */
  palSetGroupMode(GPIOA, PAL_PORT_BIT(4) | PAL_PORT_BIT(5) | PAL_PORT_BIT(6), 0, PAL_MODE_INPUT_ANALOG);
  ...
  /*
   * Starting the DAC drivers.
   */
  dacStart(&DACD1, &daccfg1);
  dacStart(&DACD2, &daccfg2);
  dacStart(&DACD3, &daccfg3);
  ...
  dacStartSendCircular(&DACD3, DAC_TABLE_SIZE, dac_buffer);
